...
const replacements = {
  ...
  'bg-executive-gold/20': 'bg-amber-100',
  'bg-executive-gold/5': 'bg-amber-50',
  ...
  'text-slate-500': 'text-slate-500', // keeping it
};
...
function processFile(filePath) {
  ...
  for (const [key, value] of Object.entries(replacements)) {
    // Only replace whole words (for tailwind classes)
    const regex = new RegExp(`(?<=[\\s"'\\\`])` + key + `(?=[\\s"'\\\`])`, 'g');
    content = content.replace(regex, value);
  }
  // also fix some specific cases where it might not have spaces
